...
// запрос из скрипта браузер к этому сервису не пропускает.
document.getElementById('send').onclick = () => {
  ...
  // Успех подтверждаем возвратом кадра на наш домен: если сервис не
  // принял отправку, он останется у себя и мы честно об этом скажем.
  let tries = 0;
